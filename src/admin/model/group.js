...
module.exports = class extends think.Model {
  ...
  async getUserGroupList({name, page, size, userId}) {
    ...
    const list = await model.field(['gb.*', 'date_format(gb.end_date, \'%Y-%m-%d\') end_date_format', 'gb.bill_id billId', 'b.name bill_name', 'c.name city_name', 'p.name province_name', 'u.name supplier_name'])
      .join({
        table: 'citys',
        join: 'inner',
        as: 'c',
