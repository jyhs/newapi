...
module.exports = class extends think.Model {
  ...
  sumGroup(from, to, userId) {
    return this.query(`select date_format(g.end_date, '%Y-%m') date,IFNULL(sum(c.sum+c.freight),0) sum from cart c,group_bill g where  g.id=c.group_bill_id and c.is_pay=1  and  ${userId ? 'g.user_id=' + userId : '1=1'} and g.end_date  BETWEEN '${from}' AND DATE_ADD('${to}',INTERVAL 1 DAY) group by date_format(g.end_date, '%Y-%m')`);
  }
  sumThisWeekGroup(userId) {
    return this.query(`select IFNULL(sum(c.sum+c.freight),0) sum from cart c,group_bill g where  g.id=c.group_bill_id and c.is_pay=1 and g.status=0 and  ${userId ? 'g.user_id=' + userId : '1=1'} and YEARWEEK(date_format(c.insert_date,'%Y-%m-%d')) = YEARWEEK(now())`);
  }
  sumLastWeekGroup(userId) {
    return this.query(`select IFNULL(sum(c.sum+c.freight),0)sum from cart c,group_bill g where  g.id=c.group_bill_id and c.is_pay=1 and g.status=0 and  ${userId ? 'g.user_id=' + userId : '1=1'} and YEARWEEK(date_format(c.insert_date,'%Y-%m-%d')) = YEARWEEK(now())-1`);
  }
  sumThisMonthGroup(userId) {
    return this.query(`select IFNULL(sum(c.sum+c.freight),0) sum from cart c,group_bill g where  g.id=c.group_bill_id and c.is_pay=1 and g.status=0 and  ${userId ? 'g.user_id=' + userId : '1=1'} and DATE_FORMAT( c.insert_date, '%Y%m' ) = DATE_FORMAT( now(), '%Y%m' )`);
  }
  sumLastMonthGroup(userId) {
    return this.query(`select IFNULL(sum(c.sum+c.freight),0) sum from cart c,group_bill g where  g.id=c.group_bill_id and c.is_pay=1 and g.status=0 and  ${userId ? 'g.user_id=' + userId : '1=1'} and DATE_FORMAT( c.insert_date, '%Y%m' ) = DATE_FORMAT( DATE_SUB(now(), INTERVAL 1 YEAR), '%Y%m' )`);
  }
  detailGroup(id) {
  ...
  async getUserGroupList({name, page, size, userId}) {
    ...
    for (const item of list.data) {
      ...
      const sumObj = await this.model('cart').field(['sum(sum+freight) sum']).where({'group_bill_id': item['id'], 'is_pay': 1}).find();
